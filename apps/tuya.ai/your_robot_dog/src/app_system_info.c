...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
        app_display_send_msg(TY_DISPLAY_TP_CHAT_STAT, (uint8_t *)tm_str, strlen(tm_str));
#endif
    }
}
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
    app_display_send_msg(TY_DISPLAY_TP_STAT_NET, &system_info.last_net_status, sizeof(system_info.last_net_status));
#endif

    // Set the initial status
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
    app_display_send_msg(TY_DISPLAY_TP_CHAT_STAT, (uint8_t *)INITIALIZING, strlen(INITIALIZING));
    app_display_send_msg(TY_DISPLAY_TP_EMOJI, (uint8_t *)"NATURAL", strlen("NATURAL"));
#endif
