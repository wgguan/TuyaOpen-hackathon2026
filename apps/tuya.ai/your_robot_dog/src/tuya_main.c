...
OPERATE_RET audio_dp_obj_proc(dp_obj_recv_t *dpobj)
{
    ...
    for (index = 0; index < dpobj->dpscnt; index++) {
        ...
        case DPID_MOVE: {
            /* Map enum/string DP to robot actions and dispatch to action thread */
            TUYA_ROBOT_ACTION_E action = ROBOT_ACTION_NONE;
            const char *move_str = NULL;
            /* Retrieve enum string via device DP schema to keep consistent with cloud definitions */
            dp_node_t *node = dp_node_find_by_devid(dpobj->devid, DPID_MOVE);
            if (node && dp->type == PROP_ENUM) {
                int idx = (int)dp->value.dp_enum;
                if (idx >= 0 && idx < node->prop.prop_enum.cnt) {
                    move_str = node->prop.prop_enum.pp_enum[idx];
                }
            } else if (dp->type == PROP_STR) {
                move_str = dp->value.dp_str;
            }
            if (move_str) {
                if (0 == strcmp(move_str, "forward")) {
                    action = ROBOT_ACTION_FORWARD;
                } else if (0 == strcmp(move_str, "backward")) {
                    action = ROBOT_ACTION_BACKWARD;
                } else if (0 == strcmp(move_str, "left")) {
                    action = ROBOT_ACTION_LEFT;
                } else if (0 == strcmp(move_str, "right")) {
                    action = ROBOT_ACTION_RIGHT;
                } else if (0 == strcmp(move_str, "spin")) {
                    action = ROBOT_ACTION_SPIN;
                } else if (0 == strcmp(move_str, "stand")) {
                    action = ROBOT_ACTION_STAND;
                } else if (0 == strcmp(move_str, "sit")) {
                    action = ROBOT_ACTION_SIT;
                } else if (0 == strcmp(move_str, "get_down")) {
                    action = ROBOT_ACTION_GETDOWN;
                } else if (0 == strcmp(move_str, "handshake")) {
                    action = ROBOT_ACTION_HANDSHAKE;
                } else if (0 == strcmp(move_str, "dance")) {
                    action = ROBOT_ACTION_DANCE;
                } else if (0 == strcmp(move_str, "jump")) {
                    action = ROBOT_ACTION_JUMP;
                } else if (0 == strcmp(move_str, "stretch")) {
                    action = ROBOT_ACTION_STRETCH;
                } else {
                    PR_WARN("Unknown move enum: %s", move_str);
                }
            } else {
                PR_WARN("DPID_MOVE value not resolvable (type:%d)", dp->type);
            }
            if (action != ROBOT_ACTION_NONE) {
                tuya_robot_action_set(action);
                PR_NOTICE("Robot action triggered by DP: %s", move_str ? move_str : "<none>");
            }
            break;
        }
        default:
