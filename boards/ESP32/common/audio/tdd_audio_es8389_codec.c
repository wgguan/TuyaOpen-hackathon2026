...
static void enable_input_device(bool enable)
{
    if (enable) {
        ...
        PR_DEBUG("enable_input_device");
    } else {
        PR_DEBUG("disable_input_device");
...
static OPERATE_RET tkl_i2s_es8389_send(void *buff, uint32_t len)
{
    // len -> data len
...
static int tkl_i2s_es8389_recv(void *buff, uint32_t len)
{
    // len -> data len
    esp_err_t ret = ESP_OK;
...
static void esp32_i2s_es8389_read_task(void *args)
{
    ESP_I2S_ES8389_HANDLE_T *hdl = (ESP_I2S_ES8389_HANDLE_T *)args;
    if (NULL == hdl) {
        PR_ERR("I2S es8389 read task args is NULL");
        return;
    }
    for (;;) {
        // Read data from I2S es8389
        int data_len = tkl_i2s_es8389_recv(hdl->data_buf, hdl->data_buf_len);
        if (data_len < 0) {
            PR_ERR("I2S es8389 read failed with error code: %d", data_len);
            tal_system_sleep(I2S_READ_TIME_MS);
            continue;
        }
        if (data_len == 0) {
            PR_DEBUG("I2S es8389 read 0 bytes.");
            tal_system_sleep(I2S_READ_TIME_MS);
            continue;
        }
...
static OPERATE_RET __tdd_audio_esp_i2s_es8389_open(TDD_AUDIO_HANDLE_T handle, TDL_AUDIO_MIC_CB mic_cb)
{
    ...
    TDD_AUDIO_ES8389_CODEC_T *cfg = &hdl->cfg;
    if (NULL == cfg) {
        PR_ERR("I2S es8389 cfg is NULL");
        return OPRT_COM_ERROR;
    }

    TUYA_CALL_ERR_RETURN(codec_es8389_init(cfg));

    // data buffer
    hdl->data_buf_len = I2S_READ_TIME_MS * cfg->mic_sample_rate / 1000;
    hdl->data_buf_len = hdl->data_buf_len * sizeof(int16_t);
    PR_DEBUG("I2S es8389 recv buffer len: %d", hdl->data_buf_len);
    ...
    if (NULL == hdl->mutex_play) {
        PR_ERR("I2S es8389 mutex create failed");
        return OPRT_COM_ERROR;
    }
    ...
    TUYA_CALL_ERR_LOG(
        tal_thread_create_and_start(&hdl->thrd_hdl, NULL, NULL, esp32_i2s_es8389_read_task, (void *)hdl, &thread_cfg));

    return rt;
}

static OPERATE_RET __tdd_audio_esp_i2s_es8389_play(TDD_AUDIO_HANDLE_T handle, uint8_t *data, uint32_t len)
{
    OPERATE_RET rt = OPRT_OK;
    ...
    if (NULL == data || len == 0) {
        PR_ERR("I2S es8389 play data is NULL");
        return OPRT_COM_ERROR;
    }

    tal_mutex_lock(hdl->mutex_play);

    TUYA_CALL_ERR_LOG(tkl_i2s_es8389_send(data, len));
...
static OPERATE_RET __tdd_audio_esp_i2s_es8389_config(TDD_AUDIO_HANDLE_T handle, TDD_AUDIO_CMD_E cmd, void *args)
{
    OPERATE_RET rt = OPRT_OK;
...
static OPERATE_RET __tdd_audio_esp_i2s_es8389_close(TDD_AUDIO_HANDLE_T handle)
{
    OPERATE_RET rt = OPRT_OK;
...
OPERATE_RET tdd_audio_es8389_codec_register(char *name, TDD_AUDIO_ES8389_CODEC_T cfg)
{
    ...
    memset(_hdl, 0, sizeof(ESP_I2S_ES8389_HANDLE_T));
