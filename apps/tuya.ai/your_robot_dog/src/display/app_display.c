...
***********************typedef define***********************
***********************************************************/
/* Use unified display message type from tuya_ai_display.h */
typedef TY_DISPLAY_MSG_T DISPLAY_MSG_T;
...
/* robot ui initializer provided in robot_app.c */
void tuya_robot_init(void);
/* robot ui app handler provided in robot_app.c */
...
static __attribute__((unused)) OPERATE_RET __get_ui_font(UI_FONT_T *ui_font)
{
...
static __attribute__((unused)) void __app_display_msg_handle(DISPLAY_MSG_T *msg_data)
{
    ...
    /* chat mode setter currently unused; reserved for future */
    default: {
        PR_DEBUG("Invalid display type: %d", msg_data->type);
    } break;
    }
