...
// send asr text to display
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
            app_display_send_msg(TY_DISPLAY_TP_HUMAN_CHAT, data, len);
#else
    ...
    case AI_AUDIO_EVT_AI_REPLIES_TEXT_DATA: {
        ...
        if (ai_text_len >= AI_AUDIO_TEXT_SHOW_LEN) {
            app_display_send_msg(TY_DISPLAY_TP_AI_CHAT, p_ai_text, ai_text_len);
            ai_text_len = 0;
        }
#endif
#else
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
                app_display_send_msg(TY_DISPLAY_TP_EMOJI, (uint8_t *)emo->name, strlen(emo->name));
#endif
            }
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
        app_display_send_msg(TY_DISPLAY_TP_EMOJI, (uint8_t *)EMOJI_NEUTRAL, strlen(EMOJI_NEUTRAL));
        app_display_send_msg(TY_DISPLAY_TP_CHAT_STAT, (uint8_t *)STANDBY, strlen(STANDBY));
#else
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
        app_display_send_msg(TY_DISPLAY_TP_CHAT_STAT, (uint8_t *)LISTENING, strlen(LISTENING));
#else
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
        app_display_send_msg(TY_DISPLAY_TP_STAT_SPEAK, (uint8_t *)SPEAKING, strlen(SPEAKING));
#else
